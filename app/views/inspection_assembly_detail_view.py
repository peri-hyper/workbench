# inspection_assembly_detail_view.py
...
from app.models import InspectionAssemblyDetailRecord, InspectionAssemblyRecord  # ★

class InspectionAssemblyDetailView(object):
    """
    组装检验 - 记录详情 API 视图
    (原 InspectionCuttingDetailView)
    """

    def query(self, request):
        """
        查询 InspectionAssemblyDetailRecord 列表（分页 & 简单搜索）
        ...
        try:
            ...
            # 假设前端以 { "inspection_number": "xxx" } 形式传进搜索
            # 这里写一段逻辑获取 inspection_number
            ins_number = request.data.get('search_data', {})

            query_set = InspectionAssemblyDetailRecord.objects.filter(
    ...
    def addRecord(self, request):
        ...
        try:
            ...
            for i, item in enumerate(data_list):
                ...
                # 外键：inspection_number => InspectionAssemblyRecord
                ins_num_str = item.get("inspection_number_id", "")
                if not ins_num_str:
                    logger.error("inspection_number_id missing, skip")
                    continue
                try:
                    assembly_record = InspectionAssemblyRecord.objects.get(inspection_number=ins_num_str)
                except InspectionAssemblyRecord.DoesNotExist:
                    logger.error(f"Record not found for inspection_number={ins_num_str}, skip this item.")
                    continue
                ...
                new_record = InspectionAssemblyDetailRecord.objects.create(
                    ...
                    image_2=image_2,
                    image_3=image_3,
                    inspection_number=assembly_record,
                    pointX=pointX,
                    pointY=pointY,
                )
    ...
    def deleteRecord(self, request):
        ...
        try:
            ...
            InspectionAssemblyDetailRecord.objects.filter(**idmap).delete()
    ...
    def editRecord(self, request):
        ...
        try:
            ...
            InspectionAssemblyDetailRecord.objects.filter(**idmap).update(**editForm)
